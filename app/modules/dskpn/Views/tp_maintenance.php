...
<script>
  ...
  const tpDatas = <?= json_encode(!empty($tp_session) ? $tp_session : []); ?> //ni leh buang jakgi
...
<script>
function getTPFromDSKPCode(sbm_id)
{
  let dskpCode = $("#dskp-code-subject-" + sbm_id).val();
  $.ajax({
    url: getTPListEndpoint + "?dskp_code=" + dskpCode,
    type: 'GET',
    dataType: 'json',
      ...
      if(response.status = 'success')
      {
        if(response.data.length > 0)
        {
          ...
          Swal.fire({
            icon: "error",
            title: "Maaf!",
            text: "Tiada rekod Tahap Penguasaan bagi Kod '" + dskpCode + "'"
          });
        }
      } else {
        Swal.fire({
